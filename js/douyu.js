...
let siteKey = 'http://api.maiyoux.com:81/mf';
let siteType = 0;
...
class DouyuSpider extends Spider {
    ...
    constructor() {
        super();
        this.siteUrl = 'http://api.maiyoux.com:81/mf';
    }

    getName() { return "🐳|斗鱼直播" }

    getAppName() { return "斗鱼直播" }

    getJSName() { return "douyu" }

    async requestRaw(reqUrl, method) {
    ...
    async request(reqUrl) {
        let resRaw = await this.requestRaw(reqUrl)
        return resRaw.data;
    }
    ...
    async init(inReq, _outResp) {
        ...
        if (dataFrom == 'justlive') {
            ...
            this.siteUrl = 'https://m.douyu.com';
        }
        return {};
    }
    ...
    async category(inReq, _outResp) {
        ...
        if (isJustLive) {
            if (tid == 'home') {
                url = this.siteUrl + '/api/live/getRecommendByPlatform?platform=douyu&size=20&page=' + pg;
            } else {
                url = this.siteUrl + '/api/live/getRecommendByPlatformArea?platform=douyu&size=20&area=' + tid + '&page=' + pg;
            }
            ...
            if (tid == 'home') {
                url = this.siteUrl + '/api/room/list?page=' + pg;
            } else {
                url = this.siteUrl + '/api/room/list?type=' + extend.cateId + '&page=' + pg;
            }
            const data = await this.request(url);
    ...
    async detail(inReq, _outResp) {
        ...
        for (const id of ids) {
            const resp = await this.request('http://live.yj1211.work/api/live/getRoomInfo?platform=douyu&roomId=' + id);
    ...
    async search(inReq, _outResp) {
        ...
        const data = await this.requestRaw(this.siteUrl + '/api/search/liveRoom?did=10000000000000000000000000001501&limit=40&offset=0&sk=' + wd, 'post');

...
个人修改：
    1、增加多个临时邮箱后缀以及4个随机邮箱接口，自主选择
    2、脚本修改作者GitHub：https://github.com/LiJunYi2
    3、脚本基于Atong管理员基础上进行修改。
    4、已新增客户端version多个版本以及版本对应Salts值，感谢作者【纸鸢花的花语】的视频教程。
"""
...
def get_random_version():
    # 从version_list中提取所有版本号
    version_numbers = [v_info["v"] for v_info in saltsList]
    # 随机选择一个版本号
...
def start():
    ...
    email_code = input("输入要使用的临时邮箱(1-4)：")
    ...
    if not email_code:
        # 如果没有输入，则随机赋值1-4
        email_code = str(random.randint(1, 4))
...
if __name__ == '__main__':
    ...
    print('5、脚本二改已有4个随机邮箱接口和邮箱后缀，脚本修改作者：https://github.com/LiJunYi2。')
    print('6、临时邮箱分流规则：https://raw.githubusercontent.com/LiJunYi2/QuantumultX/main/rule/pikpak_email.list')
    print('7、20240405更新，新增多个PikPak版本及Salts值，感谢原作者的教程视频！')
    print('8、输入1-4，不输入则随机选择临时邮箱。')
    start()
    input('程序运行结束')

...
class HuyaSpider extends Spider {
    ...
    getAppName() { return "虎牙直播" }

    getJSName() { return "huya" }

    async requestRaw(reqUrl, headers) {
    ...
    async request(reqUrl) {
        ...
        let resRaw = await this.requestRaw(reqUrl, defHeader);
        return resRaw.data;
    }
    ...
    async detail(inReq, _outResp) {
        ...
        for (const id of ids) {
            ...
            if (isJustLive) {
                ...
                const resp = await this.requestRaw('https://www.huya.com/' + id, headers);
                let liveData = null;
                ...
                if (streamInfo) {
                    ...
                    if (streamInfo) {
                        ...
                        liveData = JSON.parse(this.base64Decode(liveDataBase64));
                    }
                }
                ...
                const data = await this.request('https://mp.huya.com/cache.php?m=Live&do=profileRoom&roomid=' + id);
    ...
    getPlayUrlData(streamInfo) {
        ...
        const fm = this.unquote(cryptoInfo.fm);
        const fmDecoded = this.base64Decode(fm);
    ...
    async search(inReq, _outResp) {
        ...
        let page = pg || 1;
        if (page == 0) page = 1;
        const data = await this.request('https://search.cdn.huya.com/?m=Search&do=getSearchContent&q=' + wd + '&uid=0&v=4&typ=-5&livestate=0&rows=40&start=0');
        // const data = JSON.parse(resp);
        let videos = [];
